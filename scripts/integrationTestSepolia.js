...
async function main() {
  ...
  const stableAddr = process.env.STABLE_ADDR;
  const mockETHAddr = process.env.MOCKETH_ADDR;
  const exchangeAddr = process.env.EXCHANGE_ADDR;
  ...
  console.log("Swapping 1 ETH to LMC...");
  ...
  console.log("Stable gained:", ethers.utils.formatEther(deltaStable));
  console.log("ETH spent:", ethers.utils.formatEther(deltaETH));

  const diff = deltaStable.sub(expectedLMC);
  const pctError = diff.mul(10000).div(expectedLMC).toNumber() / 100;
